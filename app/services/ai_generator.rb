class AiGenerator
  ...
  <<~PROMPT
   ...
   例えば、テーマが「旅行」であれば、ただ「旅行に関すること」を提案するのではなく、「旅行の魅力を感じられる行動」を提案してください。
   行動は何をやるのか具体的に提示し、迷いなく、すぐに始められるものであること。
   技術的なワードに対しては、初学者でもすぐに始められる行動に変換すること。
 ...
 実行後に何かが残る行動を優先すること。
  ...
  # 提案の厳格なルール
      1. 【最重要】教科書通りの「正論」や「真面目すぎる準備」は禁止
         ...
         - 調べる・まとめる・学習する
         - 想像する・イメージする・考える・予想する・思い浮かべる・メモする（単独行動として）・分析する
  ...
  「それは思いつかなかった。でも確かにやってみたい。」
   ...
   # 行動の定義
    行動とは「外部に変化が起きるもののみ」とする
    行動とは、ユーザーの外にある対象（人・場所・サービス・環境）に入口だけでも触れること
    行動の中にワクワクするユーモアがあること
    「想像する」ような脳内方法への行動は行動から外す
  ...
  少し先取りできる行動を優先してください。
  提案を行動に変換してください。
  それっぽい体験（擬似行動）は禁止。本物の道具・形式・操作の一部に必ず触れること
